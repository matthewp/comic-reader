import momentumView from './momentum.js';
...
class PinchZoom extends HTMLElement {
  constructor() {
      ...
      const pointerTracker = new PointerTracker(this, {
          start: (pointer, event) => {
            this._startTime = Date.now();
              ...
              this._updateMomentum({ down: pointer });
              return true;
          },
      ...
      this._stageElSizeChange = this._stageElSizeChange.bind(this);
      this.addEventListener('wheel', event => this._onWheel(event));
      this.addEventListener('transitionend', this._onAnimationEnd);

      let self = this;
      this._resizeObserver = new ResizeObserver(this._stageElSizeChange);
  ...
  _stageElChange() {
      ...
      this._updateMomentum = momentumView({
        source: this._positioningEl,
        friction: 0.94,
        update: (x, y) => {
          const { d: scale, e: ox, f: oy } = this._transform;
          const startScale = this._startScale;
          const pointereHasEnded = startScale === null;
      ...
      this._positioningEl.addEventListener('draw', this._stageElSizeChange);
  ...
  _stageElSizeChange() {
    ...
    let positioningRect = this._positioningEl.getBoundingClientRect();
    let { e: x, f: y } = this._transform;

    let minX = 0 - (positioningRect.width - thisRect.width);
    let minY = 0 - (positioningRect.height - thisRect.height);
  ...
  _onPointerMove(previousPointers, currentPointers) {
      ...
      this._updateMomentum({ move: currentPointers[0] });
  ...
  _onPointerEnd(pointer, event) {
    ...
    this._updateMomentum({ up: pointer });
    this._checkBounds();
  ...
  _checkBounds() {
    const thisBounds = this.getBoundingClientRect();
    const positioningElBounds = this._positioningEl.getBoundingClientRect();
    const { right: thisRight, left: thisLeft, top: thisTop, bottom: thisBottom, height: thisHeight } = thisBounds;
    const { right: elRight, left: elLeft, top: elTop, bottom: elBottom, height: elHeight } = positioningElBounds;
  ...
  _checkSize() {
    const { d: scale } = this._transform;
    const scaleChanged = scale !== this._startScale;

    if(scaleChanged) {
      this._stageElSizeChange();
    }
  }
