...
class ZipBook {
  ...
  async load() {
    let entries = await getEntries(this.src);
  ...
  canAdvance() {
    return this.nextPageNumber() <= this.getLength();
  }

  canAdvanceTo(pageNumber) {
    return pageNumber <= this.getLength();
  }
  ...
  async preload() {
    if(this.getLength() > this.urls.length) {
